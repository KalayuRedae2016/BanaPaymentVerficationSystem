<template>
  <div class="p-4">
    ...
    <div v-if="!nothingToPay && !getReceipt" class="py-6 -mt-1 border-t border-blue-500">
      <div class="text-gray-800 font-semibold bg-blue-300 py-4 text-md ">
        <p>
           <i class="fas fa-chevron-right mr-3 text-pink-500 ml-3"></i>User Code:<span class="text-blue-900 text-sm ml-5">{{
            userCode }}
            </span>
        </p>
        <p>
          <i class="fas fa-chevron-right mr-3 text-pink-500 ml-3"></i> Full Name:
          <span class="text-indigo-800 text-sm ml-5">{{ fullName }}</span>
        </p>
      </div>

      <div class="border-t border-blue-900 ml-5 mt-5">
        <div class="p-4 border-b border-blue-900 cursor-pointer" v-for="(payment, paymentIndex) in payments"
          :key="paymentIndex" @click="changePaymentSelection(payment.id, paymentIndex)" :class="[
          ...
          <div class="mt-4 flex flex-col space-y-5">
            <div class="flex flex-row items-center">
              <div class="flex flex-col">
                <p class="text-indigo-800 text-2xl font-extrabold">
                  <i class="fas fa-chevron-right mr-3 text-pink-500"></i>
                  <span class="text-lg">Month:{{ payment.activeMonthInString }}</span>
                </p>
              ...
              <table class="w-full bg-white">
                <thead>
                  <tr class="bg-gray-200">
                    <th class="w-24 px-3 text-sm font-extrabold tracking-wide text-left text-indigo-800">
                      Metric
                    </th>
                    <th class="w-24 px-3 text-sm font-extrabold tracking-wide text-left text-indigo-800">
                      Amount
                    ...
                    <th class="w-32 p-3 text-sm font-extrabold tracking-wide text-left text-indigo-800">
                      Bank Type
                    </th>
                    <th class="w-32 p-3 text-sm font-extrabold tracking-wide text-left text-indigo-800">
                      TT Number
                ...
                <tbody>
                  <tr>
                    <td class="p-3  text-sm text-gray-700 whitespace-nowrap">
                      <p class="bg-blue-100 px-2 text-indigo-800 font-extrabold rounded-lg">Regular</p>
                    </td>
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                    ...
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      ...
                      <input v-if="!payment.regular.isPaid" type="Date" class="custom-input h-7"
                        @change="fetchRegularPenality(payment)" :value="payment.regular.paidAt"
                        @input="payment.regular.paidAt = $event.target.value" />
                      <p v-if="payment.verifyRegularPaymentDate" class="text-red-500">
                    ...
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      ...
                      <select v-if="!payment.regular.isPaid"
                        class="custom-select w-64 text-sm px-3 py-2.5 leading-tight" v-model="payment.regular.bankType">
                      ...
                      <p v-if="payment.verifyRegularBankType" class="text-red-500">
                    ...
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      ...
                      <input v-if="!payment.regular.isPaid" type="text" class="custom-input h-7"
                        v-model="payment.regular.TTNumber" />
                      <p v-if="payment.verifyRegularTTNumber" class="text-red-500">
                    ...
                    <td v-if="!payment.regular.isPaid" class="p-3 text-sm text-gray-700 whitespace-nowrap relative">
                      <!-- Confirmation Text Above Button -->
                      <div v-if="paymentIndex === blockConfirmedIndex"
                      ...
                      <button @click="
                        ...
                        @mouseenter="showBlockConfirmUnConfirmText(paymentIndex,'conShow')" @mouseleave="hideBlockConfirmUnConfirmText('conhide')">
                        <span class="flex items-center justify-center bg-white rounded-full p-1">
                          <!-- Circular white background -->
                          <i class="fas fa-check-circle text-pink-500"></i> <!-- Pink check icon -->
                        </span>
                    ...
                    <td v-if="payment.regular.isPaid" class="p-3 text-sm text-gray-700 whitespace-nowrap relative">
                      ...
                      <button @click="
                        ...
                        " class="custom-button bg-blue-500 text-white flex items-center p-2 rounded hover:bg-blue-600"
                        @mouseenter="showBlockConfirmUnConfirmText(paymentIndex,'unconShow')" @mouseleave="hideBlockConfirmUnConfirmText('unconhide')">
                        <span class="flex items-center justify-center bg-white rounded-full p-1">
                  ...
                  <tr v-if="payment.subsidy.amount > 0">
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      <p class="bg-cyan-100 px-2 text-indingo-800 font-extrabold rounded-lg">Subsidy</p>
                    </td>
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                     
                     
                      <input v-if="!payment.subsidy.isPaid" readonly type="text" class="custom-input h-7" :value="payment.subsidy.amount"
                        @input="payment.subsidy.amount = $event.target.value" />

                      <p v-else >{{ payment.subsidy.amount }}</p>
                    </td>
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      <p v-if="payment.subsidy.isPaid">{{ payment.subsidy.paidAt }}</p>
                     
                      <input v-if="payment.subsidy.amount > 0 && !payment.subsidy.isPaid" type="Date" class="custom-input h-7"
                        @change="fetchSubsidyPenality(payment)" :value="payment.subsidy.paidAt"
                        @input="payment.subsidy.paidAt = $event.target.value" />
                      
          
                        <p v-if="payment.verifySubsidyPaymentDate" class="text-red-500">
                        Payment Date is required
                      </p>
                    ...
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      <p v-if="payment.subsidy.isPaid">{{ payment.subsidy.bankType }}</p>
                      <select v-if="payment.subsidy.amount > 0 && !payment.subsidy.isPaid"
                        class="custom-select w-64 text-sm px-3 py-2.5 leading-tight" v-model="payment.subsidy.bankType">
                        <option value="" disabled selected>Bank Type</option>
                        <option value="LIB">LIB</option>
                        <option value="WEGAGEN">WEGAGEN</option>
                        <option value="CBE">CBE</option>
                      </select>
                      <p v-if="payment.verifySubsidyBankType" class="text-red-500">
                        Bank Type is required
                      </p>
                      <p v-if="payment.subsidy.amount == 0">
                        -------------------------
                    ...
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      <p v-if="payment.subsidy.isPaid">{{  payment.subsidy.TTNumber}}</p>
                      <input v-if="payment.subsidy.amount > 0 && !payment.subsidy.isPaid" type="text" class="custom-input h-7"
                        v-model="payment.subsidy.TTNumber" />
                      <p v-if="payment.verifySubsidyTTNumber" class="text-red-500">
                        TTNumber is required
                      </p>
                      <p v-if="payment.subsidy.amount == 0">
                        -------------------------
                    ...
                    <td v-if="!payment.subsidy.isPaid" class="p-3 text-sm text-gray-700 whitespace-nowrap relative">
                      <!-- Confirmation Text Above Button -->
                      <div v-if="paymentIndex === subsidyConfirmedIndex"
                        class="absolute top-0 left-1/2 transform -translate-x-1/2 mb-1 bg-blue-800 text-white text-xs px-2 py-1 rounded"
                        style="margin-top: 0.5rem; margin-left: 2rem">
                        Confirm
                      </div>
                      ...
                      <button @click="
                        ...
                        " class="custom-button bg-blue-500 text-white flex items-center p-2 rounded hover:bg-blue-600"
                        @mouseenter="showSubsidyConfirmUnConfirmText(paymentIndex,'conShow')" @mouseleave="hideSubsidyConfirmUnConfirmText('conhide')">
                        <span class="flex items-center justify-center bg-white rounded-full p-1">
                          <!-- Circular white background -->
                          <i class="fas fa-check-circle text-pink-500"></i> <!-- Pink check icon -->
                        </span>
                    ...
                    <td v-if="payment.subsidy.isPaid" class="p-3 text-sm text-gray-700 whitespace-nowrap relative">
                      <!-- Confirmation Text Above Button -->
                      <div v-if="subsidyUnconfirmIndex"
                        class="absolute top-0 left-1/2 transform -translate-x-1/2 mb-1 bg-blue-800 text-white text-xs px-2 py-1 rounded"
                        style="margin-top: 0.5rem; margin-left: 2.25rem">
                        Unconfirm
                      </div>
                      ...
                      <button @click="
                        ...
                        " class="custom-button bg-blue-500 text-white flex items-center p-2 rounded hover:bg-blue-600"
                        @mouseenter="showSubsidyConfirmUnConfirmText(paymentIndex,'unconShow')" @mouseleave="hideSubsidyConfirmUnConfirmText('unconhide')">
                        <span class="flex items-center justify-center bg-white rounded-full p-1">
                          <i class="fas fa-times text-pink-500"></i>
                          <!-- Change icon color to pink -->
                        </span>
                  ...
                  <tr v-if="payment.urgent.amount > 0">
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      <p class="bg-pink-100 px-2 text-indigo-800 font-extrabold rounded-lg">Urgent</p>
                    </td>
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                       <p v-if="payment.urgent.isPaid">{{  payment.urgent.amount  }}</p>
                       <input v-if="!payment.urgent.isPaid" readonly type="text" class="custom-input h-7" :value="payment.urgent.amount"
                        @input="payment.urgent.amount = $event.target.value" />
                    </td>
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      <p v-if="payment.urgent.isPaid">{{ payment.urgent.paidAt }}</p>
                      <input v-if=" !payment.urgent.isPaid" type="Date" class="custom-input h-7"
                        @change="fetchUrgentPenality(payment)" :value="payment.urgent.paidAt"
                        @input="payment.urgent.paidAt = $event.target.value" />
                      <p v-if="payment.verifyUrgentPaymentDate" class="text-red-500">
                        Payment Date is required
                      </p>
                    ...
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      <p v-if="payment.urgent.isPaid">{{ payment.urgent.bankType }}</p>
                      <select v-if="payment.urgent.amount > 0 && !payment.urgent.isPaid"
                        class="custom-select w-64 text-sm px-3 py-2.5 leading-tight" v-model="payment.urgent.bankType">
                        <option value="" disabled selected>Bank Type</option>
                        <option value="LIB">LIB</option>
                        <option value="WEGAGEN">WEGAGEN</option>
                        <option value="CBE">CBE</option>
                      </select>
                      <p v-if="payment.verifyUrgentBankType" class="text-red-500">
                        Bank Type is required
                      </p>
                      <p v-if="payment.urgent.amount == 0">
                        -------------------------
                    ...
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      <p v-if="payment.urgent.isPaid">{{ payment.urgent.TTNumber }}</p>
                      <input v-if="payment.urgent.amount > 0 && !payment.urgent.isPaid" type="text" class="custom-input h-7"
                        v-model="payment.urgent.TTNumber" />
                      <p v-if="payment.verifyUrgentTTNumber" class="text-red-500">
                        TTNumber is required
                      </p>
                      <p v-if="payment.urgent.amount == 0">
                        -------------------------
                    ...
                    <td v-if="!payment.urgent.isPaid" class="p-3 text-sm text-gray-700 whitespace-nowrap relative">
                      <!-- Confirmation Text Above Button -->
                      <div v-if="paymentIndex === urgentConfirmedIndex"
                        class="absolute top-0 left-1/2 transform -translate-x-1/2 mb-1 bg-blue-800 text-white text-xs px-2 py-1 rounded"
                        style="margin-top: 0.5rem; margin-left: 2rem">
                        Confirm
                      </div>
                      ...
                      <button @click="
                        ...
                        " class="custom-button bg-blue-500 text-white flex items-center p-2 rounded hover:bg-blue-600"
                        @mouseenter="showUrgentConfirmUnConfirmText(paymentIndex,'conShow')" @mouseleave="hideUrgentConfirmUnConfirmText('conhide')">
                        <span class="flex items-center justify-center bg-white rounded-full p-1">
                          <!-- Circular white background -->
                          <i class="fas fa-check-circle text-pink-500"></i> <!-- Pink check icon -->
                        </span>
                    ...
                    <td v-if="payment.urgent.isPaid" class="p-3 text-sm text-gray-700 whitespace-nowrap relative">
                      <!-- Confirmation Text Above Button -->
                      <div v-if="urgentUnconfirmedIndex"
                        class="absolute top-0 left-1/2 transform -translate-x-1/2 mb-1 bg-blue-800 text-white text-xs px-2 py-1 rounded"
                        style="margin-top: 0.5rem; margin-left: 2.25rem">
                        Unconfirm
                      </div>
                      ...
                      <button @click="
                        ...
                        " class="custom-button bg-blue-500 text-white flex items-center p-2 rounded hover:bg-blue-600"
                          @mouseenter="showUrgentConfirmUnConfirmText(paymentIndex,'unconShow')" @mouseleave="hideUrgentConfirmUnConfirmText('unconhide')">
                        <span class="flex items-center justify-center bg-white rounded-full p-1">
                          <i class="fas fa-times text-pink-500"></i>
                          <!-- Change icon color to pink -->
                        </span>
                  ...
                  <tr>
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      <p class="bg-yellow-100 px-2 text-indigo-800 font-extrabold rounded-lg">Service</p>
                    </td>
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      <p v-if="payment.service.isPaid">
                        {{ payment.service.amount }}
                      </p>
                      <input v-if="!payment.service.isPaid" type="text" class="custom-input h-7 w-32"
                        :value="payment.service.amount" @input="payment.service.amount = $event.target.value" />
                    </td>
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      ...
                      <input v-if="!payment.service.isPaid" type="Date" class="custom-input h-7"
                        :value="payment.service.paidAt" @input="payment.service.paidAt = $event.target.value" />
                      <p v-if="payment.verifyServicePaymentDate" class="text-red-500">
                        Payment Date is required
                      </p>
                    </td>
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      ...
                      <select v-if="!payment.service.isPaid"
                        class="custom-select w-64 text-sm px-3 py-2.5 leading-tight" v-model="payment.service.bankType">
                        <option value="" disabled selected>Bank Type</option>
                        <option value="LIB">LIB</option>
                        <option value="WEGAGEN">WEGAGEN</option>
                        <option value="CBE">CBE</option>
                      </select>
                      <p v-if="payment.verifyServiceBankType" class="text-red-500">
                        Bank Type is required
                      </p>
                    </td>
                    ...
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      ...
                      <input v-if="!payment.service.isPaid" type="text" class="custom-input h-7"
                        v-model="payment.service.TTNumber" />
                      <p v-if="payment.verifyServiceTTNumber" class="text-red-500">
                        TTNumber is required
                      </p>
                    </td>
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                    ...
                    <td v-if="!payment.service.isPaid" class="p-3 text-sm text-gray-700 whitespace-nowrap relative">
                      <!-- Confirmation Text Above Button -->
                      <div v-if="paymentIndex === serviceConfirmedIndex"
                        class="absolute top-0 left-1/2 transform -translate-x-1/2 mb-1 bg-blue-800 text-white text-xs px-2 py-1 rounded"
                        style="margin-top: 0.5rem; margin-left: 2rem">
                        Confirm
                      </div>
                      ...
                      <button @click="
                        ...
                        " class="custom-button bg-blue-500 text-white flex items-center p-2 rounded hover:bg-blue-600"
                        @mouseenter="showServiceConfirmUnConfirmText(paymentIndex,'conShow')" @mouseleave="hideServiceConfirmUnConfirmText('conhide')">
                        <span class="flex items-center justify-center bg-white rounded-full p-1">
                          <!-- Circular white background -->
                          <i class="fas fa-check-circle text-pink-500"></i> <!-- Pink check icon -->
                        </span>
                    ...
                    <td v-if="payment.service.isPaid" class="p-3 text-sm text-gray-700 whitespace-nowrap relative">
                      <!-- Confirmation Text Above Button -->
                      <div v-if="serviceUnconfirmIndex"
                        class="absolute top-0 left-1/2 transform -translate-x-1/2 mb-1 bg-blue-800 text-white text-xs px-2 py-1 rounded"
                        style="margin-top: 0.5rem; margin-left: 2.25rem">
                        Unconfirm
                      </div>
                      ...
                      <button @click="
                        ...
                        " class="custom-button bg-blue-500 text-white flex items-center p-2 rounded hover:bg-blue-600"
                        @mouseenter="showServiceConfirmUnConfirmText(paymentIndex,'unconShow')" @mouseleave="hideServiceConfirmUnConfirmText('unconhide')">
                        <span class="flex items-center justify-center bg-white rounded-full p-1">
                          <i class="fas fa-times text-pink-500"></i>
                          <!-- Change icon color to pink -->
                        </span>
            ...
            <div class="overflow-x-auto">
              <table class="w-full bg-white">
                ...
                <tbody>
                  <tr>
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      <p class="bg-red-200 px-2 text-indigo-800 font-extrabold rounded-lg">Penality</p>
                    </td>
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                    ...
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      {{  payment.regular.penality + payment.subsidy.penality +  payment.urgent.penality }}
                    </td>

                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
            
                      <input v-if="(payment.regular.penality + payment.subsidy.penality +  payment.urgent.penality) > 0 && !payment.penality.isPaid " type="Date" class="custom-input h-7"
                     
                        v-model="payment.penality.paidAt"
                        @input="payment.penality.paidAt = $event.target.value" />

                        <p v-if="(payment.regular.penality + payment.subsidy.penality +  payment.urgent.penality) > 0 && payment.penality.isPaid ">{{ payment.penality.paidAt }}</p>
                         <p v-if="(payment.regular.penality + payment.subsidy.penality +  payment.urgent.penality)==0">-------</p>
                        <p v-if="payment.verifyPenalityPaymentDate" class="text-red-500">
                           Payment Date is required
                      </p>
                    ...
                    <td  class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      <select v-if="(payment.regular.penality + payment.subsidy.penality + payment.urgent.penality) > 0" class="custom-select w-64 text-sm px-3 py-2.5 leading-tight"
                         v-model="payment.penality.bankType"
      
                          @input="payment.penality.bankType = $event.target.value"
                         >
                     ...
                     <p v-else>--------</p>
                     <p v-if="payment.verifyPenalityBankType" class="text-red-500">
                          Bank Type is required
                      </p>
                    </td>

                     <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      <input v-if="(payment.regular.penality + payment.subsidy.penality + payment.urgent.penality) > 0" type="text" class="custom-input h-7"
                        v-model="payment.penality.TTNumber" 
                         @input="payment.penality.TTNumber = $event.target.value" 
                       />
                     <p v-else>--------</p>
                     <p v-if="payment.verifyPenalityTTNumber" class="text-red-500">
                          TTNumber is required
                      </p>
                    </td>
                    ...
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      <p v-if="(payment.regular.amount + payment.subsidy.penality + payment.urgent.penality)>0">{{ payment.penality.isPaid }}</p>
                      <p v-else>--------</p>
                    </td>


                    <td v-if="!payment.penality.isPaid" class="p-3 text-sm text-gray-700 whitespace-nowrap relative">
                      <div v-if="paymentIndex === penalityConfirmedIndex"
                        class="absolute top-0 left-1/2 transform -translate-x-1/2 mb-1 bg-blue-800 text-white text-xs px-2 py-1 rounded"
                        style="margin-top: 0.5rem; margin-left: 2rem">
                        Confirm
                      </div>
                      <button 
                      v-if="(payment.regular.penality + payment.subsidy.penality + payment.urgent.penality) > 0"
                      @click="
                        ...
                        " class="custom-button bg-blue-500 text-white flex items-center p-2 rounded hover:bg-blue-600"
                        @mouseenter="showPenalityConfirmUnConfirmText(paymentIndex,'conShow')" @mouseleave="hidePenalityConfirmUnConfirmText('conhide')">
                        <span class="flex items-center justify-center bg-white rounded-full p-1">
                          <i class="fas fa-check-circle text-pink-500"></i> <!-- Pink check icon -->
                        </span>
                      </button>
                      <p v-else class="bg-pink-100 text-indigo-800 p-1 font-extrabold">Action Not Needed</p>
                    </td>
                    <td v-if="payment.penality.isPaid" class="p-3 text-sm text-gray-700 whitespace-nowrap relative">
                      <p  class="bg-pink-100 text-indigo-800 p-1 font-extrabold">Action Not Needed</p>
                    </td>
                  </tr>
                </tbody>
              </table>
              <p v-if="payment.confirmRegularSubsisyUrgentFirst" class="text-red-500 ml-5 mt-3">
    ...
    <div v-if="nothingToPay" class="py-6 px-6 -mt-1 border-t border-blue-500 w-full mb-64">
      <div
        ...
        <a @click="userLevelReport()" href="#"
          class="text-sm w-1/4 text-blue-600 hover:text-blue-800 font-medium text-lg ml-4">
          See Payment History
        </a>
        <div class="absolute top-0 right-0 -mt-3 -mr-3 w-6 h-6 bg-green-500 rounded-full"></div>
      </div>
    </div>
    <div v-if="showPaymentDetailModal">
      <transition name="fade" mode="out-in">
        <div class="fixed inset-0 flex items-center justify-center z-10 bg-black bg-opacity-50 rounded-lg">
          ...
          <div class="bg-white rounded-lg-lg p-6 border border-cyan-500">
            <div class="fixed inset-0 flex items-center justify-center z-50">
              <div class="bg-white rounded-lg-lg shadow-lg p-8 w-96">
                ...
                <button @click="confirmedSuccessfully = !confirmedSuccessfully"
  ...
  <transition name="fade" mode="out-in">
    <div class="fixed inset-0 flex items-center justify-center z-10 bg-black bg-opacity-50">
      <!-- Modal Content -->
      <div class="bg-white rounded-lg p-6 border border-cyan-800 shadow-lg w-96">
        <!-- Modal Header -->
        <div class="flex items-center justify-between bg-indigo-800 text-white rounded-t-lg px-4 py-2">
          <h2 class="text-lg font-semibold">Payment Confirmation</h2>
          <i class="fas fa-times cursor-pointer" @click="showConfirmModal = false"></i>
        </div>
        ...
        <div class="p-6">
          ...
          <p class="text-red-500" v-if="paymentUnconfirm">
            Do you want to unconfirm this payment with the following details?
          </p>

          <div class="ml-4 mt-4">
            <div v-if="paymentType === 'regular'" class="space-y-2">
              <p><i class="fas fa-coins text-yellow-500"></i> Regular Amount: {{ regularPayment.amount }}</p>
              <p><i class="fas fa-hashtag text-blue-500"></i> Regular TTNumber: {{ regularPayment.TTNumber }}</p>
              <p><i class="fas fa-university text-green-500"></i> Regular Bank Type: {{ regularPayment.bankType }}</p>
              <p><i class="fas fa-calendar-alt text-purple-500"></i> Regular Payment Date: {{ regularPayment.paidAt }}</p>
            </div>
            <div v-else-if="paymentType === 'subsidy'" class="space-y-2">
              <p><i class="fas fa-coins text-yellow-500"></i> Subsidy Amount: {{ subsidyPayment.amount }}</p>
              <p><i class="fas fa-hashtag text-blue-500"></i> Subsidy TTNumber: {{ subsidyPayment.TTNumber }}</p>
              <p><i class="fas fa-university text-green-500"></i> Subsidy Bank Type: {{ subsidyPayment.bankType }}</p>
              <p><i class="fas fa-calendar-alt text-purple-500"></i> Subsidy Payment Date: {{ subsidyPayment.paymentDate }}</p>
            </div>
            <div v-else-if="paymentType === 'urgent'" class="space-y-2">
              <p><i class="fas fa-coins text-yellow-500"></i> Urgent Amount: {{ urgentPayment.amount }}</p>
              <p><i class="fas fa-hashtag text-blue-500"></i> Urgent TTNumber: {{ urgentPayment.TTNumber }}</p>
              <p><i class="fas fa-university text-green-500"></i> Urgent Bank Type: {{ urgentPayment.bankType }}</p>
              <p><i class="fas fa-calendar-alt text-purple-500"></i> Urgent Payment Date: {{ urgentPayment.paidAt }}</p>
            </div>
            <div v-else-if="paymentType === 'service'" class="space-y-2">
              <p><i class="fas fa-coins text-yellow-500"></i> Service Amount: {{ servicePayment.amount }}</p>
              <p><i class="fas fa-hashtag text-blue-500"></i> Service TTNumber: {{ servicePayment.TTNumber }}</p>
              <p><i class="fas fa-university text-green-500"></i> Service Bank Type: {{ servicePayment.bankType }}</p>
              <p><i class="fas fa-calendar-alt text-purple-500"></i> Service Payment Date: {{ servicePayment.paidAt }}</p>
            </div>
            <div v-else-if="paymentType === 'penality'" class="space-y-2">
              <p><i class="fas fa-coins text-yellow-500"></i> Penalty Amount: {{ penalityPayment.amount }}</p>
              <p><i class="fas fa-hashtag text-blue-500"></i> Penalty TTNumber: {{ penalityPayment.TTNumber }}</p>
              <p><i class="fas fa-university text-green-500"></i> Penalty Bank Type: {{ penalityPayment.bankType }}</p>
              <p><i class="fas fa-calendar-alt text-purple-500"></i> Penalty Payment Date: {{ penalityPayment.paidAt }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="flex justify-end space-x-4 mt-6">
          <button @click="yesConfirmed()" class="bg-indigo-500 hover:bg-indigo-600 text-white py-2 px-4 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 flex items-center">
            <i class="fas fa-check mr-2"></i> OK
          </button>
          <button @click="showConfirmModal = false" class="bg-gray-300 hover:bg-gray-400 text-gray-700 py-2 px-4 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500 flex items-center">
...
l;
  ...
  console.log("payload",this.payload);
    ...
    ) 
    {
      ...
      if(this.payments[paymentIndex].penality.bankType === null || this.payments[paymentIndex].penality.bankType == ''){
    ...
    fetchRegularPenality(selectedPayment) {
      ...
      this.$apiClient
        ...
        .then((response) => {
          console.log("response regular penality", response.data);
          this.payments.forEach((payment) => {
