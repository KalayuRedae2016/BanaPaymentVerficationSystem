<template>
  <div class="">
    <form class="px-5 py-5">
      <h2 class="text-2xl font-bold mb-6">Subsidy Payment</h2>
      <div class="mb-4">
        <label
      ...
      <div class="mb-4">
        ...
        <select
          v-model="subsidy.bankType"
          id="startDay"
          class="w-full sm:w-full md:w-full lg:w-full xl:w-3/4 px-4 py-2 border border-gray-500 rounded-lg mt-2 ml-3 focus:border-indigo-800"
        >
          <option value="" disabled>Select Bank Type</option>
          <option
            v-for="subsidyBankAccount in subsidyBankAccounts"
            :key="subsidyBankAccount._id"
            :value="subsidyBankAccount._id"
          >
            {{ subsidyBankAccount.bankType }}
          </option>
        </select>
      </div>
      ...
      <div class="mb-4">
        ...
        <input
          ...
          id="bank_account"
          class="w-full sm:w-full md:w-full lg:w-full xl:w-3/4 px-4 py-2 border border-gray-500 rounded-lg mt-2 ml-3 focus:border-indigo-800"
          v-model="subsidy.depositedFromBankAccount"
        />
      </div>
      ...
      <div class="mb-4">
        ...
        <input
          ...
          id="bank_account"
          class="w-full sm:w-full md:w-full lg:w-full xl:w-3/4 px-4 py-2 border border-gray-500 rounded-lg mt-2 ml-3 focus:border-indigo-800"
          v-model="subsidy.bankReferenceId"
        />
      </div>
      <div class="mb-4">
        <label
        ...
        <div class="bg-white rounded-lg p-6 border border-cyan-500">
          <div class="font-extrabold text-xl mt-2 mb-4">
            <div class="flex flex-row">
              ...
              <div class="ml-16 mt-2" @click="subsidyModal">
                <svg
            ...
            <div class="flex mb-4">
              <div class="w-1/2">
                <div class="flex flex-row">
                  ...
                  <p class="ml-10 text-gray-900 text-2xl">{{ subsidy.amount }}</p>
                </div>
                <div class="flex flex-row">
                  <label class="block text-2xl font-medium text-gray-700"
                    >Deposit Date:</label
                ...
                <div class="flex flex-row">
                  ...
                  <p class="ml-10 text-gray-900 text-2xl">
                    {{ subsidy.depositedFromBankAccount }}
                  </p>
                </div>
                <div class="flex flex-row">
                  <label class="block text-2xl font-medium text-gray-700"
                ...
                <div class="flex flex-row">
                  ...
                  <p class="ml-10 text-gray-900 text-2xl">{{ subsidy.month }}</p>
                </div>
              </div>
              <div class="w-1/2"></div>
            </div>
   ...
   // alert("on mounted");
    ...
    this.apiClient.get("/api/v1/organization").then((response) => {
      ...
      if (response.data !== null) {
        this.subsidyBankAccounts = response.data.organization.blockBankAccounts;
      } else {
  ...
  methods: {
    insertSubsidyData() {
      ...
      const regularHistoryData = {
        ...
        debitAmount: this.subsidy.amount,
        depositDate: this.subsidy.depositDate,
        companyBankTypeId: this.subsidy.bankType,
        debitAccountNumber: this.subsidy.depositedFromBankAccount,
        bankReferenceId: this.subsidy.bankReferenceId,
        month: this.subsidy.month,
      };
