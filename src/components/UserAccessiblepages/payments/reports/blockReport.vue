<template>
  <div class="p-6 w-full sm:w-auto rounded">
    ...
    <h1 class="text-blue-800 text-md font-bold mt-5 mb-5">Overdue Blocks</h1>
...
export default {
  ...
  created() {
    // this.apiClient.post("/block-payments").then((response) => {
    //     console.log("response",response);
    //     this.allBlockPayments=response.data;
    //   });
  },
  mounted() {
  ...
  methods: {
    ...
    fetchBlockData() {
      this.$apiClient.post("/api/v1/closeHistory").then((response) => {
        console.log("response", response);
        this.blockPaymentsYear = response.data;
      });
    },
